...
rl.on('line', (line) => {
  ...
  // DP방식으로 n번째의 피보나치 값을 가져오는 함수. topdown
  const getFibonacciUsingDPTopDown = (n) => {
    // 0번째 1번째 피보나치 수
    if (n <= 1) return n
    // 메모가 있는지 확인한다.
    if (memo[n] != 0) return memo[n]
    memo[n] = getFibonacciUsingDPTopDown(n - 1) + getFibonacciUsingDPTopDown(n - 2)
    return memo[n]
  }

  // DP방식으로 n번째의 피보나치 값을 가져오는 함수. bottom-up
  const getFibonacciUsingDPBottomUp = (n) => {
    // 0번째 1번째 피보나치 수
    if (n <= 1) return n
    // 메모가 있는지 확인한다.
    if (memo[n] != 0) return memo[n]
    memo[n] = getFibonacciUsingDPBottomUp(n - 1) + getFibonacciUsingDPBottomUp(n - 2)
    return memo[n]
  }
  ...
  arr.forEach((val) => {
    ...
    result.push(getFibonacciUsingDPTopDown(val))
  })
  console.log(result);
  process.exit()
})
